...
def plot_trials(csv):
    ...
    #ast.literal_eval just evaluates that the entry is indeed a dictionary. Reference: https://stackoverflow.com/questions/15197673/using-pythons-eval-vs-ast-literal-eval
	data['good'] = (data['good_actions'] * 1.0 / \
                    (data['initial_deadline'] - data['final_deadline'])).rolling(window=10, center=False).mean()# multiply by 1.0 here to make it to a float so that divide will work in Python 2

...
class TrafficLight(object):
    ...
    def update(self, t):
        if t - self.last_updated >= self.period:
            self.state = not self.state  # Assuming state is boolean ### THAT MEANS THIS ONE TOGGLES?!
            self.last_updated = t
...
class Environment(object):
    ...
    # Inputs to the agent (through sense function), and corresponding valid actions out of the agent.
    valid_actions = [None, 'forward', 'left', 'right']
    valid_inputs = {'light': TrafficLight.valid_states, 'oncoming': valid_actions, 'left': valid_actions, 'right': valid_actions}
    valid_headings = [(1, 0), (0, -1), (-1, 0), (0, 1)]  # E, N, W, S => check this again maybe it's # E, S, W, N
    ...
    def __init__(self, verbose=False, num_dummies=100, grid_size = (8, 6)):
        ...
        self.done = False
        self.t = 0
        self.agent_states = OrderedDict() #Ordered dictionaries are just like regular dictionaries but they remember the order that items were inserted. When iterating over an ordered dictionary, the items are returned in the order their keys were first added.
        self.step_data = {}
        self.success = None
    ...
    def create_agent(self, agent_class, *args, **kwargs):
        ...
        self.agent_states[agent] = {'location': random.choice(self.intersections.keys()), 'heading': (0, 1)} #all agents facing North first
        return agent
    ...
    def reset(self, testing=False):
        ...
        # Initialize agent(s).. Each agent has its value as below as a dictionary.
        for agent in self.agent_states.iterkeys():
            ...
            # intersections and headings still in 'positions'
                ...
                # Now delete the taken location and heading from 'positions' if agent not primary agent
    ...
    def sense(self, agent):
        ...
        #Python's assert statement helps you find bugs more quickly and with less pain. This note has some suggestions on good ways to use it. Reference: https://wiki.python.org/moin/UsingAssertionsEffectively
    ...
    def act(self, agent, action):
        ...
        # deadline is distance_away * 5. Literally it's the timestep remaining. So when deadline=0, fnc = 1, which means highest penalty!
        ...
        # Agent wants to drive left:
            ...
            if light != 'green': # Running a red light
                violation = 2 # Major violation
                if inputs['left'] == 'forward' or inputs['right'] == 'forward': # Cross traffic, #there should be another one here with inputs['oncoming'] == 'forward'
                    violation = 4 # Accident
